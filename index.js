  ...
  if (channelName === 'cayo-lfg') {
    ...
    if (content.startsWith(PREFIX)) {
      ...
      if (command === 'myblacklist') {
        ...
        } catch (e) { message.reply('Blacklist system not available, Kapitan.'); }
        return;
      }
      ...
      if (command === 'unblock') {
        try {
          ...
          if (!target) return message.reply('Kapitan, mention someone: `?unblock @user`');
          await blacklist.removeFromBlacklist(message.author.id, target.id);
          message.reply(`✅ **${target.username}** can join your heists again, Kapitan.`);
        } catch (e) { message.reply('Could not unblock, Kapitan.'); }
        return;
      }
